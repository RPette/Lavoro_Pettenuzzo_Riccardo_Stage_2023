...
def Calculate_Width_Cut(segment_center, height):
    ...
    cv2.line(thresh, (x_mid, y_mid), (x_mid-2, y_mid-2), (255, 0, 0), 2)
    
    x_opposite, y_opposite = 0, 0

    #find the coords of the dot on the opposite side of the cut
    for i in range(height):
        if thresh[x_mid, i] == 0 and thresh[y_mid, i+1] == 255:
            x_opposite = i
            y_opposite = y_mid
            break
    cv2.line(thresh, (x_opposite, y_opposite), (x_opposite+5, y_opposite+5), (0, 255, 0), 2)
    ...
    if x1 != x2:
        ...
        cv2.line(thresh, (x_opposite, y_opposite), (x_inters, y_inters), (0, 0, 255), 2)
        return Calculate_Width(x_inters, x_opposite, y_inters, y_opposite)
    else:
        cv2.line(thresh, (x_mid, y_mid), (x_opposite, y_opposite), (0, 0, 255), 2)
        return x_opposite-x_mid




slab = cv2.imread(r"C:\Users\stage.upe4\Downloads\lastra_carta.jpg")
...
#TODO check if deleting the background can help to solve this problem cause it's possible that with a static value for the threshold not all the black dots will be eliminated
...
for v in segments.values():
    print("segment's center", v)
    print(Calculate_Width_Cut(v, height))
